...
<script>
    ...
    const handleSubmit = async () => {
      try {
        ...
        await updatePerformance(performance.id, updatedPerformance);
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-4">
    ...
    <div>
      ...
      <input id="seat_count" type="number" bind:value={performance.seat_count} required class="w-full p-2 border rounded">
    </div>
    <div>
      <label for="min_price" class="block mb-2">최저 가격</label>
      <input id="min_price" type="number" step="0.01" bind:value={performance.min_price} required class="w-full p-2 border rounded">
    </div>
    <div>
      <label for="max_price" class="block mb-2">최고 가격</label>
      <input id="max_price" type="number" step="0.01" bind:value={performance.max_price} required class="w-full p-2 border rounded">
    </div>
